			...
			if(checkLogin(result)){
				if(result.handle=="success"){
					if(status==2){
						...
						os.html("交易成功<div class=\"text-right\"><a onclick=\"return confirm('确定要删除这个订单吗？')\" href=\"${pageContext.request.contextPath}/order/delOrder?orderId="+orderId + ">删除订单</a></div>");
					}
					else if(status==6){
						os.html("已提交退货申请，请等待商家处理");
					}
					else if(status==9){
						os.html("交易关闭<div class=\"text-right\"><a onclick=\"return confirm('确定要删除这个订单吗？')\" href=\"${pageContext.request.contextPath}/order/delOrder?orderId="+orderId+">删除订单</a></div>");
					}
					$("#msgTitle").html("操作成功");
					$("#msgBody").html("订单操作成功");
					$("#msgModal").modal();	
