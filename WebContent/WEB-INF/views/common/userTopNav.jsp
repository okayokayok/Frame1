...
function logout() {
	$.post(
					...
					function() {
						$("#info")
								.html(
										"<li id='li1'><span>游客您好，欢迎来到专卖店！</span>"
...
function showCart() {
	...
	if (goodsList != null && goodsList.length > 0) {
		...
		for (var i = 0; i < goodsList.length; i++) {
			...
			s += "<td><a href='" + getContextPath()
					+ "/goods/getGoodsDetailById?goodsId=" + goods.id
					+ "&cateId=" + goods.cateId + "'>" + goods.name
					+ "</a><br>" + goods.size + "&nbsp;&nbsp;"
					+ goods.color + "</td>";
			s += "<td>原价<span class='glyphicon glyphicon-yen' aria-hidden='true'></span>"
					+ goods.price
					+ "<br>现售<span class='glyphicon glyphicon-yen' aria-hidden='true'></span>"
					+ goods.discount + "</td>";
			s += "<td><div class='input-group input-group-sm col-xs-5'><input class='form-control input-small' onblur='changeCart("
					+ i
					+ ")' id='cartnum' name='num' type='number' min='1' value='"
					+ goods.num + "'/></div></td>";
			s += "<td><span class='glyphicon glyphicon-yen' aria-hidden='true'></span>"
		...
		s += "<div class='text-right'>已选"
				...
				+ "件商品&nbsp;&nbsp;合计（不含运费）<span class='glyphicon glyphicon-yen' aria-hidden='true'></span>"
			...
			<nav class="navbar navbar-default navbar-static-top" role="navigation">
				...
				<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
					...
					<form class="navbar-form navbar-left form-inline " role="search" method="post" action="${pageContext.request.contextPath}/goods/searchGoods">
						...
						<%-- <a href="${pageContext.request.contextPath}/cate/addView">添加类别</a> --%>
					</form>
